...
-- Add room_url column to bookings
ALTER TABLE public.bookings ADD COLUMN room_url text;

-- Replace book_session to generate Jitsi room URL
...
BEGIN
  ...
  -- Generate unique Jitsi room URL with recording enabled
  _room_url := 'https://meet.jit.si/YozoraSession-' || replace(_booking_id::text, '-', '');
